...
def create_top15_plot(df):
    ...
    # Store the plot
    top_plot = plt.gcf()  # Get current figure
    return top_plot

def create_50plus1_plot(df):
    # Calculate the sum of each row for numerical columns
    df['RowSum'] = df.iloc[:, 5:].sum(axis=1)
    ...
    # Store the plot
    fifty_plus_one_plot = plt.gcf()  # Get current figure
    return fifty_plus_one_plot
...
def plot_graphs(output_file):
    ...
    # Create plots
    top_plot = create_top15_plot(df)
    fifty_plus_one_plot = create_50plus1_plot(df)

    # Show plots in separate windows
    top_plot.show()
    fifty_plus_one_plot.show()

    # Prompt user to save plots
    save_option = input("¿Desea guardar las gráficas (s/n)? ").strip().lower()
    if save_option == 's':
        # Save top 15 dianas plot
        top_plot.savefig(os.path.splitext(output_file)[0] + '_top15.png')
        print(f"Gráfico 'Top 15 Dianas' guardado como {os.path.splitext(output_file)[0] + '_top15.png'}")

        # Save Gráfica 50+1 plot
        fifty_plus_one_plot.savefig(os.path.splitext(output_file)[0] + '_50plus1.png')
        print(f"Gráfico 'Gráfica 50+1' guardado como {os.path.splitext(output_file)[0] + '_50plus1.png'}")

        # Close plots to release memory
        plt.close(top_plot)
        plt.close(fifty_plus_one_plot)
